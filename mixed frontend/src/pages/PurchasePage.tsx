...
import type { AppData, PurchaseRecord, GodownAllocation } from "../store";
...
interface Props {
  ...
  currentUser: string;
  viewOnlyRecord?: PurchaseRecord | null;
}
...
export default function PurchasePage({ data, setData, currentUser, viewOnlyRecord }: Props) {
  ...
  const handleSave = () => {
    ...
    if (!form.supplierName || form.godownAllocations.length === 0 || !form.ratePerBag) {
      setShowErrors(true);
      return;
    }
    ...
    let newData: AppData;
    if (editingId) {
      newData = { ...data, purchases: data.purchases.map((p) => (p.id === editingId ? record : p)) };
      newData = addChangeLog(newData, currentUser, "Updated", "Purchase", `Updated purchase from ${form.supplierName}`);
    } else {
      newData = { ...data, purchases: [record, ...data.purchases] };
      newData = addChangeLog(newData, currentUser, "Added", "Purchase", `New purchase from ${form.supplierName}, ${totalBags} bags, ₹${totalAmount.toFixed(2)}`);
    }
  ...
  const handleDelete = (id: string) => {
    ...
    let newData = { ...data, purchases: data.purchases.filter((p) => p.id !== id) };
    newData = addChangeLog(newData, currentUser, "Deleted", "Purchase", `Deleted purchase from ${rec?.supplierName}`);
    setData(newData);
  };
    ...
    return (
      <div className="flex flex-col h-full bg-gray-50">
        ...
        <div className="bg-green-600 text-white px-4 py-3 flex items-center gap-3 flex-shrink-0">
          ...
          <h2 className="text-lg font-semibold">{editingId ? "Edit Purchase" : "Purchase Form"}</h2>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-3">
          ...
          {/* Supplier Name */}
          <div className={`bg-white border rounded-xl p-3 ${showErrors && !form.supplierName ? 'border-red-500 ring-1 ring-red-500' : 'border-gray-200'}`}>
            <label className="block text-xs text-gray-500 mb-2 font-medium">Supplier Name</label>
            <SearchableDropdown
              label="Supplier Name"
          ...
          <div className={`bg-white border rounded-xl p-3 ${showErrors && form.godownAllocations.length === 0 ? 'border-red-500 ring-1 ring-red-500' : 'border-gray-200'}`}>
            <label className="block text-xs text-gray-500 mb-2 font-medium">
              Godowns <span className="text-green-600">(select multiple, enter bags for each)</span>
            </label>
            <GodownAllocator
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="flex items-center justify-between px-4 py-3 border-b border-gray-700 flex-shrink-0">
        <div>
          <h2 className="text-lg font-semibold text-white">Purchase</h2>
      ...
      {data.purchases.length > 0 && (
        <div className="flex gap-3 px-4 py-2 bg-[#0f1923] border-b border-gray-700 flex-shrink-0">
          <div className="text-xs text-gray-400">
            Total: <span className="text-white font-medium">{data.purchases.reduce((s, p) => s + p.totalBags, 0)} bags</span>
          </div>
          <div className="text-xs text-gray-400">
            Amount: <span className="text-green-400 font-medium">₹{new Intl.NumberFormat('en-IN').format(data.purchases.reduce((s, p) => s + p.totalAmount, 0))}</span>
          </div>
        </div>
      )}
      ...
      <div className="flex-1 overflow-y-auto p-4">
        {data.purchases.length === 0 ? (
          <div className="text-center py-16 text-gray-500">
            ...
            <p className="text-sm mt-1 text-gray-500">Click "New Purchase" to add one</p>
          </div>
        ) : (
          <div className="space-y-3">
            {data.purchases.map((p) => (
              <div key={p.id} className="bg-[#1e2a3a] border border-gray-700 rounded-xl p-4 hover:border-green-600 transition-colors">
                <div className="flex justify-between items-start mb-2">
                  <div>
                    <p className="font-semibold text-white">{p.supplierName}</p>
                    <p className="text-xs text-gray-400">{p.date}</p>
                  </div>
                  <div className="text-right">
                    <p className="font-bold text-green-400">₹{new Intl.NumberFormat('en-IN').format(p.totalAmount)}</p>
                    <p className="text-xs text-gray-400">{p.totalBags} bags</p>
                  </div>
                </div>
                <div className="flex items-center gap-2 text-xs text-gray-400 flex-wrap">
                  {p.type && <span className="bg-gray-700 px-2 py-0.5 rounded">{p.type}</span>}
                  <span>₹{new Intl.NumberFormat('en-IN').format(p.ratePerBag)}/{p.bagSize}kg</span>
                  {p.chitInKg > 0 && <span className="text-orange-400">Chit: {p.chitInKg}kg</span>}
                  {p.godownAllocations.map((a) => (
                ...
                <div className="flex justify-between items-center mt-2 pt-2 border-t border-gray-700/50 text-xs text-gray-500">
                  ...
                  <div className="flex gap-3">
                    <button onClick={() => handleEdit(p)} className="text-blue-400 hover:text-blue-300 transition-colors">Edit</button>
                    <button onClick={() => handleDelete(p.id)} className="text-red-400 hover:text-red-300 transition-colors">Delete</button>
                  </div>
                </div>
              </div>
            ))}
